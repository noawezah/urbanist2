...
export default function MenuBrowser({ categories, items }: { categories: MenuCategory[]; items: MenuItem[] }) {
 ...
 useEffect(()=>{
  const sync=()=>{const id=window.location.hash.slice(1);const found=categories.find(cat=>cat.id===id);if(found){setGroup(found.group);setCategoryId(found.id);setQuery('');}};
 ...
 function choose(id:string){
  ...
  if(sticky.current&&sticky.current.getBoundingClientRect().top<0){
   const header=document.querySelector('.public-header')?.getBoundingClientRect().height||88;
   window.scrollTo({top:sticky.current.offsetTop-header,behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});
  }
 }
 ...
 return <>
  ...
  <div className="menu-results" ref={panel}>
   ...
   <div role={query.trim()?'region':'tabpanel'} id={query.trim()?'search-results':`panel-${categoryId}`} aria-labelledby={query.trim()?undefined:`tab-${categoryId}`} aria-label={query.trim()?'Search results':undefined} tabIndex={0} className="menu-item-grid">
    {results.map(item=><button key={item.id} className="menu-item" aria-haspopup="dialog" aria-label={`${item.name}, ${priceLabel(item)}. View details`} onClick={()=>setSelected(item)}><span className="menu-item-top"><span className="menu-item-name">{item.name}</span><span className="menu-item-price">{priceLabel(item)}</span></span>{query.trim()&&<span className="menu-item-category">{categories.find(c=>c.id===item.categoryId)?.name}</span>}<span className="menu-item-description">{item.description||item.serving||'Discover the details.'}</span><span className="menu-item-bottom"><span>{item.serving||categories.find(c=>c.id===item.categoryId)?.hours||'Made at The Urbanist'}</span><span className="item-open"><span>Details</span><RiArrowRightUpLine size={20}/></span></span></button>)}
   </div>
